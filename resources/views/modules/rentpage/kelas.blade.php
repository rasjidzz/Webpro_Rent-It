...
<div class="container my-5">
    <form action="/rentKelas" method="POST" enctype="multipart/form-data">
        @csrf
        <section class="row gap-4 justify-content-center">
            <div id="box1" class="col-sm-4 w-30 rounded-3 border border-1 border-secondary px-0">
                {{-- <img src="/Assets/Fasilitas/{{ $gedung->slug }}-2.jpg" class="card-img-top"> --}}
                @foreach (explode(', ', $gedung->image) as $index => $imagePath)
                    <img src="{{ asset('storage/' . $imagePath) }}" class="card-img-top rounded" alt="{{ $gedung->slug }}-{{ $index + 1 }}" width="300" height="300">
                    @break
                @endforeach
                <div class="card-body">
                    <h4 class="card-title text-center">{{ $gedung->name }}</h4>
                    <p>
                        Kelas : {{ $kelas->room }}
                    </p>
                    <p>
                        <span class="formatted-price" data-raw-price="{{ $gedung->price }}">
                            Rp {{ $gedung->price }}
                        </span>
                    </p>
            ...
            <div id="box2" class="col-sm-6 w-40 rounded-3 border border-1 border-secondary">
                ...
                <div class="mb-3 ">
                    ...
                    <input readonly type="text" value={{ $tanggalSewa }} class="form-control" name="tanggalSewa" readonly>
                ...
                <input hidden type="number" class="form-control" id="user_id" name="user_id"
                    value="{{ auth()->user()->id }}">
                <input hidden type="number" class="form-control" id="gedung_id" name="gedung_id"
                    value={{ $gedung->id }}>
                <input type="number" hidden id="kelas_id" name="kelas_id" 
                    value={{ $kelas->id }}>
            </div>
        </section>
